...
| `locals.tf` | Naming conventions and computed values | **Flat projects**: Centralized naming patterns<br>**Modules**: Each module has its own |
...
**Locals File (`locals.tf`):**

**For flat/simple projects:**
- Define naming conventions and name patterns in root `locals.tf`
- Centralize all resource naming logic for consistency
- Example:
  ```hcl
  locals {
    ...
    bucket_name = "${var.project}-${var.region}-databricks-root"
  }
  ```

**For modular projects:**
- **Root `locals.tf`**: Optional, only for root-level computed values
- **Module-specific `locals.tf`**: Each module has its own locals for internal naming
- **Pass naming inputs as variables** to modules (e.g., `project`, `environment`)
...
#### F. Comment and Documentation Standards

**Section Separators:**

Use decorative separators for major sections in each `.tf` file:
...
- To separate major functional groups within a file
- Before groups of related resources (e.g., "VPC Configuration", "Security Groups", "Subnets")
- Keep section titles concise without additional descriptive comments

**Example file structure with separators:**
...
**Inline Comments:**
- Use simple `#` comments for explanations and clarifications
- Place comments **above** the line they describe
- Keep comments concise and meaningful
- Use inline comments sparingly for brief clarifications on the same line

```hcl
# Security group for Databricks workspace
resource "aws_security_group" "databricks" {
  name_prefix = "${var.project}-databricks-"
  ...
  tags = {
    Name = "${var.project}-databricks-sg"
  }
}
```

**Output Organization:**
- **Always** place outputs in a dedicated `outputs.tf` file
- Group related outputs under section separators (e.g., "Network Outputs", "Databricks Outputs", "PrivateLink Outputs")
- Organize outputs alphabetically within each section
- Include descriptive `description` for every output
...
1. **Data sources** (if any) at the beginning
2. **Resources** organized by function with section separators between groups
3. Each major resource type gets its own section separator
4. Keep related resources together under the same section

**Formatting Rules:**
- One blank line between top-level resource blocks
- Section separators have one blank line before and after
...
2. **Verify formatting:**
   ...
   - Verify section separators (`# ========`) are used for major sections in each file
   - Confirm all outputs are in `outputs.tf` file with section separators
...
- [ ] Section separators (`# ========`) used for major sections in each file
- [ ] All outputs placed in `outputs.tf` file with section separators
...
4. **Always** use section separators (`# ========`) for major sections in each file
5. **Always** place outputs in `outputs.tf` file with section separators
6. **Never** auto-apply without reviewing the plan
7. **Always** use latest provider versions unless specified
8. **Always** document provider/module sources in comments
9. **Always** follow alphabetical ordering for variables/outputs
10. **Always** use descriptive resource names
11. **Always** include README with usage examples
12. **Always** review security implications before deployment
